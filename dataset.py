    ...
    def __getitem__(self, idx):
        ...
        while True:
            if idx >= len(self.data):  # noise example
                label = np.int64(0)
                idx = idx % len(self.data)  # get the same index like a waveform sample, just cut out another stream
...
def get_data_loaders(catalog_path, waveform_path, batch_size=64, num_workers=4, shuffle=True, test_run=False):
    if test_run:
        num_workers = 1

    training_data = SeismoDataset(catalog_path=catalog_path, waveform_path=waveform_path, split='TRAIN',
                                  test_run=test_run)
    validation_data = SeismoDataset(catalog_path=catalog_path, waveform_path=waveform_path, split='DEV',
                                    test_run=test_run)

    training_loader = DataLoader(training_data, batch_size=batch_size, num_workers=num_workers, shuffle=shuffle)
    validation_loader = DataLoader(validation_data, batch_size=batch_size, num_workers=num_workers, shuffle=False)

    return training_loader, validation_loader


def get_test_loader(catalog_path, waveform_path, batch_size=64, num_workers=4, test_run=False):
    if test_run:
        num_workers = 1
    test_data = SeismoDataset(catalog_path=catalog_path, waveform_path=waveform_path, split='TEST', test_run=test_run)
